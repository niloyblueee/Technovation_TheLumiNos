...
const HeatmapLayer = ({ points }) => {
    ...
    useEffect(() => {
        ...
        const heatPoints = points
            ...
            .map((p) => [p.latitude, p.longitude, p.emergency ? 0.9 : 0.55]);
...
const WaterDepartment = () => {
    ...
    useEffect(() => {
        if (!loading && user && user.role !== 'water') {
            navigate('/');
        }
    ...
    const fetchIssues = useCallback(async () => {
        ...
        try {
            ...
            const waterIssues = (data || []).filter((issue) => {
                ...
                return departments.includes('water') && ['in_progress', 'resolved'].includes(issue.status);
            });

            const unresolved = waterIssues.filter((issue) => issue.status !== 'resolved');
            const resolved = waterIssues.filter((issue) => issue.status === 'resolved');
            ...
            console.error('Failed to fetch water issues', err);
            setError('Unable to load water incidents. Please try again.');
        } finally {
            setIsLoading(false);
        }
    }, [API_BASE]);
    ...
    if (loading) {
        return (
            <div className={styles.loadingScreen}>
                <FaClock />
                <span>Preparing water operations...</span>
            </div>
        );
    }
        ...
        <div className={styles.wrapper}>
            ...
            <div className={styles.grid}>
                ...
                >
                    <div className={styles.mapHeader}>
                        <div>
                            <h2>Infrastructure Heatmap</h2>
                            <p>Live concentration of water service issues.</p>
                        </div>
                        {selectedIssue ? (
                ...
                >
                    ...
                    {activeIssues.length === 0 && !isLoading ? (
                        <div className={styles.emptyState}>
                            <p>No active water incidents right now.</p>
                        </div>
                    ) : (
            ...
            >
                ...
                {resolvedIssues.length === 0 ? (
                    <div className={styles.emptyState}>No resolved water incidents yet.</div>
                ) : (
