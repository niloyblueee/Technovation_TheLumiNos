...
import { FaArrowRight, FaBriefcaseMedical, FaHeartbeat, FaStethoscope, FaRegClock, FaRedo, FaSignOutAlt } from 'react-icons/fa';
import styles from './HealthDepartment.module.css';
...
const HeatmapLayer = ({ points }) => {
    ...
    useEffect(() => {
        ...
        const heatPoints = points
            ...
            .map((p) => [p.latitude, p.longitude, p.emergency ? 0.92 : 0.6]);
...
const HealthDepartment = () => {
    ...
    useEffect(() => {
        if (!loading && user && user.role !== 'health') {
            navigate('/');
        }
    ...
    const fetchIssues = useCallback(async () => {
        ...
        try {
            ...
            const healthIssues = (data || []).filter((issue) => {
                ...
                return departments.includes('health') && ['in_progress', 'resolved'].includes(issue.status);
            });

            const unresolved = healthIssues.filter((issue) => issue.status !== 'resolved');
            const resolved = healthIssues.filter((issue) => issue.status === 'resolved');
            ...
            setError('Unable to load health incidents. Please try again.');
        } finally {
            setIsLoading(false);
        }
    }, [API_BASE]);
    ...
    const handleViewDetails = (issueId) => {
        navigate(`/health/issues/${issueId}`);
    };
    ...
    if (loading) {
        return (
            <div className={styles.loadingScreen}>
                <FaRegClock />
                <span>Preparing medical dashboard...</span>
            </div>
        );
    }
    ...
    return (
        <div className={styles.wrapper}>
            ...
            >
                <div>
                    <p className={styles.subtitle}>Integrated Health Command</p>
                    <h1 className={styles.title}>City Medical Response Center</h1>
                </div>
            ...
            >
                <div className={styles.statCard}>
                    ...
                    <FaBriefcaseMedical className={styles.statIcon} />
                </div>
                <div className={styles.statCard}>
                    <div className={styles.statLabel}>Stabilized</div>
                ...
                <div className={styles.statCard}>
                    ...
                    <FaStethoscope className={styles.statIcon} />
                </div>
            </motion.section>
            ...
            <div className={styles.grid}>
                ...
                >
                    <div className={styles.mapHeader}>
                        <div>
                            <h2>Medical Incident Heatmap</h2>
                            <p>Live concentration of health-response assignments.</p>
                        </div>
                        {selectedIssue ? (
                ...
                >
                    ...
                    {activeIssues.length === 0 && !isLoading ? (
                        <div className={styles.emptyState}>
                            <p>No active medical incidents right now.</p>
                        </div>
                    ) : (
            ...
            >
                <div className={styles.resolvedHeader}>
                    <h2>Stabilized Cases</h2>
                    <span>{resolvedIssues.length}</span>
                </div>
                {resolvedIssues.length === 0 ? (
                    <div className={styles.emptyState}>No stabilized incidents yet.</div>
                ) : (
