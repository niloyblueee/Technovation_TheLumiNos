...
const HeatmapLayer = ({ points }) => {
    ...
    useEffect(() => {
        ...
        const heatPoints = points
            ...
            .map((p) => [p.latitude, p.longitude, p.emergency ? 0.9 : 0.6]);
...
const PoliceDepartment = () => {
    ...
    useEffect(() => {
        if (!loading && user && user.role !== 'police') {
            navigate('/');
        }
    ...
    const fetchIssues = useCallback(async () => {
        ...
        try {
            ...
            const policeIssues = (data || []).filter((issue) => {
                ...
                return departments.includes('police') && ['in_progress', 'resolved'].includes(issue.status);
            });

            const unresolved = policeIssues.filter((issue) => issue.status !== 'resolved');
            const resolved = policeIssues.filter((issue) => issue.status === 'resolved');
            ...
            console.error('Failed to fetch police issues', err);
            setError('Unable to load police issues. Please try again.');
        } finally {
            setIsLoading(false);
        }
    }, [API_BASE]);
    ...
    if (loading) {
        return (
            <div className={styles.loadingScreen}>
                <FaRegClock />
                <span>Preparing police operations...</span>
            </div>
        );
    }
    ...
    return (
        <div className={styles.wrapper}>
            ...
            >
                ...
                <div className={styles.statCard}>
                    <div className={styles.statLabel}>Emergency</div>
            ...
            <div className={styles.grid}>
                ...
                >
                    <div className={styles.mapHeader}>
                        <div>
                            <h2>Citywide Incident Heatmap</h2>
                            <p>Live signal density of police-assigned incidents.</p>
                        </div>
                        {selectedIssue ? (
                ...
                >
                    ...
                    {activeIssues.length === 0 && !isLoading ? (
                        <div className={styles.emptyState}>
                            <p>No active incidents assigned to police.</p>
                        </div>
                    ) : (
            ...
            >
                ...
                {resolvedIssues.length === 0 ? (
                    <div className={styles.emptyState}>No resolved police incidents yet.</div>
                ) : (
