...
import { FaArrowRight, FaBurn, FaExclamationTriangle, FaFireExtinguisher, FaRegClock, FaRedo, FaSignOutAlt } from 'react-icons/fa';
import styles from './FireDepartment.module.css';
...
const HeatmapLayer = ({ points }) => {
    ...
    useEffect(() => {
        ...
        const heatPoints = points
            ...
            .map((p) => [p.latitude, p.longitude, p.emergency ? 0.95 : 0.65]);
...
const FireDepartment = () => {
    ...
    useEffect(() => {
        if (!loading && user && user.role !== 'fire') {
            navigate('/');
        }
    ...
    const fetchIssues = useCallback(async () => {
        ...
        try {
            ...
            const fireIssues = (data || []).filter((issue) => {
                ...
                return departments.includes('fire') && ['in_progress', 'resolved'].includes(issue.status);
            });

            const unresolved = fireIssues.filter((issue) => issue.status !== 'resolved');
            const resolved = fireIssues.filter((issue) => issue.status === 'resolved');
            ...
            console.error('Failed to fetch fire issues', err);
            setError('Unable to load fire incidents. Please try again.');
        } finally {
            setIsLoading(false);
        }
    }, [API_BASE]);
    ...
    if (loading) {
        return (
            <div className={styles.loadingScreen}>
                <FaRegClock />
                <span>Preparing firefighting console...</span>
            </div>
        );
    }
    ...
    return (
        <div className={styles.wrapper}>
            ...
            >
                <div>
                    <p className={styles.subtitle}>Rapid Response Command</p>
                    <h1 className={styles.title}>City Fire Intelligence Center</h1>
                </div>
            ...
            >
                <div className={styles.statCard}>
                    <div className={styles.statLabel}>Active Fires</div>
                    <div className={styles.statValue}>{stats.active}</div>
                    <FaFireExtinguisher className={styles.statIcon} />
                </div>
                <div className={styles.statCard}>
                    <div className={styles.statLabel}>Contained</div>
                    <div className={styles.statValueResolved}>{stats.resolved}</div>
                    <FaBurn className={styles.statIcon} />
                </div>
                <div className={styles.statCard}>
            ...
            <div className={styles.grid}>
                ...
                >
                    <div className={styles.mapHeader}>
                        <div>
                            <h2>Thermal Incident Heatmap</h2>
                            <p>Live signal of all fire-service assignments.</p>
                        </div>
                        {selectedIssue ? (
                ...
                >
                    ...
                    {activeIssues.length === 0 && !isLoading ? (
                        <div className={styles.emptyState}>
                            <p>No active fire incidents right now.</p>
                        </div>
                    ) : (
                                ...
                                >
                                    <div className={styles.issueMeta}>
                                        ...
                                        <span className={`${styles.statusBadge} ${issue.emergency ? styles.statusEmergency : ''}`}>
                                            {issue.emergency ? 'Emergency' : 'In progress'}
                                        </span>
                                    </div>
            ...
            >
                <div className={styles.resolvedHeader}>
                    <h2>Contained Incidents</h2>
                    <span>{resolvedIssues.length}</span>
                </div>
                {resolvedIssues.length === 0 ? (
                    <div className={styles.emptyState}>No contained incidents yet.</div>
                ) : (
